...
if (!isset($_GET['id'])) {
  header("Location: kelola_produk3.php");
  exit();
}
...
$stmt = $conn->prepare("SELECT * FROM produk3 WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$error) {
    $stmt = $conn->prepare("UPDATE produk3 SET nama=?, harga=?, stok=?, status=?, gambar=? WHERE id=?");
...
<main class="container mt-4">
  <h3 class="mb-4">Edit Produk 3</h3>

<?php if ($success): ?>
  <div class="alert alert-success"><?= $success ?></div>
  <meta http-equiv="refresh" content="1;url=kelola_produk3.php">
<?php elseif ($error): ?>
