  ...
  if (!empty($_FILES['gambar']['name'])) {
    ...
    if (in_array($imageFileType, $allowed_types)) {
      if (move_uploaded_file($_FILES["gambar"]["tmp_name"], $target_file)) {
        $stmt = $conn->prepare("INSERT INTO produk (nama, harga, stok, status, gambar) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("siiss", $nama, $harga, $stok, $status, $target_file);

        if ($stmt->execute()) {
            $success = "Produk berhasil ditambahkan.";
            // Kirim email notifikasi ke semua subscriber
            $linkProduk = "https://tricehijab.site/produk.php"; // bisa disesuaikan
...
<main class="container mt-4">
  ...
  <?php if ($success): ?>
    ...
    <meta http-equiv="refresh" content="1;url=kelola_produk.php">
  <?php elseif ($error): ?>
