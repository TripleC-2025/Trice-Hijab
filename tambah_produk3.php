  ...
  if (!empty($_FILES['gambar']['name'])) {
    ...
    if (in_array($imageFileType, $allowed_types)) {
      if (move_uploaded_file($_FILES["gambar"]["tmp_name"], $target_file)) {
        $stmt = $conn->prepare("INSERT INTO produk3 (nama, harga, stok, status, gambar) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("siiss", $nama, $harga, $stok, $status, $target_file);

        if ($stmt->execute()) {
            $success = "Produk berhasil ditambahkan.";
            // Kirim email notifikasi ke semua subscriber
            $linkProduk = "https://tricehijab.site/produk3.php"; // bisa disesuaikan
...
<main class="container mt-4">
  <h3 class="mb-4">Tambah Produk 3</h3>
