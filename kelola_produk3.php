...
// Ambil data produk
$produk = $conn->query("SELECT * FROM produk3 ORDER BY id DESC");
?>
...
<main class="container mt-4">
  <div class="d-flex justify-content-between align-items-center mb-3">
    <h3>Kelola Produk 3</h3>
    <a href="tambah_produk3.php" class="btn btn-primary"><i class="fa fa-plus"></i> Tambah Produk</a>
  </div>
        ...
        <?php if ($produk->num_rows > 0): ?>
          <?php $no = 1; while($row = $produk->fetch_assoc()): ?>
            <tr style=" text-align: center">
              ...
              <td><center><?= number_format($row['stok'], 0, ',', '.') ?></center></td>
              <td><?= htmlspecialchars($row['status']) ?></td>
              <td><img src="<?= htmlspecialchars($row['gambar']) ?>" width="60" alt="Produk"></td>
              <td>
                <a href="edit_produk3.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-warning">Edit</a>
                <a href="hapus_produk3.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Yakin ingin menghapus produk ini?')">Hapus</a>
              </td>
            </tr>
          <?php endwhile; ?>
        <?php else: ?>
